...
| MVP | Stage 01 핵심 루프와 Stage 02 장소 이전 기반을 검증하는 내부용 버티컬 슬라이스 |
| 문서 버전 | v0.1 |
| 기준일 | 2026-07-27 |

## 현재 구현 상태

현재 저장소에는 실행·저장 로드, Day 1 낮 영업과 정산, 새벽 시장·배치 준비, Day 2 영업 전환까지 이어지는 P0 핵심 루프와 좌석 3~4, 계란 메뉴, 고정 4종 주방 스테이션, 주방장·접객 고용 UI와 주급 흐름, Stage 01에서 Stage 02의 넓은 장소를 구매하는 이전 기반이 구현돼 있다.
...
- `srcs/screens/day_screen.gd`: Stage 01의 1200×1920 맵과 Stage 02의 1600×2400 넓은 맵, 고정 HUD, 탭 이동, 드래그 카메라, 낮 영업 시설과 정산 한 화면
...
- 플레이어와 직원이 같은 주문·조리대·완성 접시·결제를 노리면 먼저 작업을 예약한 쪽이 완료하거나 취소할 때까지 소유한다. 직원은 이동을 시작하기 전에 예약하고, 플레이어는 접근 작업을 시작할 때 예약한다. 예약 뒤의 도착 순서로 소유권이 바뀌지 않는다.
...
- Stage 01 정산에서는 500문으로 새 장소를 살 수 있다. 구매하면 메뉴 해금과 메뉴 레벨만 유지하고, 0문·Day 1·좌석 1·직원 없음·새 시작 재고로 Stage 02 낮 화면을 즉시 다시 만든다.
- Stage 02는 1600×2400의 넓은 경계와 새 시설·좌석 좌표를 사용하며 Stage 01에서 구매한 좌석과 고용한 직원은 남지 않는다.
...
12. [11_stage_02.md](docs/11_stage_02.md) — 새 장소 구매, 이전 데이터 계약과 넓은 Stage 02 배치
...
| 장소 이전 | Stage 01 정산에서 500문, 메뉴 해금·레벨만 유지하고 Stage 02로 초기화 |
| 전투 | 없음 |
| MVP 정의 | 스토어 출시판이 아닌, Stage 01 핵심 루프와 Stage 02 장소 이전 기반을 검증하는 내부용 버티컬 슬라이스 |
...
- 장소 이전 규칙 변경: `03_stage_01.md`, `04_progression_economy.md`, `11_stage_02.md`
