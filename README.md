...
현재 저장소에는 실행·저장 로드, 낮 화면 이동, 손님의 주문→제작→서빙→식사→결제→퇴장 반복 흐름, 첫 조리대 강화, 좌석 2와 점원 해금, 영업 타이머와 마지막 주문 처리까지 담당하는 코드 기반이 구현돼 있다.
...
- 새벽 화면과 정산 화면 전환은 아직 구현하지 않았다. Main은 목적 화면을 만들 수 없으면 현재 화면을 유지하고 오류를 기록한다.
