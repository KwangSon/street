...
현재 저장소에는 실행·저장 로드, Day 1 낮 영업과 정산, 새벽 시장·배치 준비, Day 2 영업 전환까지 이어지는 P0 핵심 루프와 P0 이후의 좌석 3~4, 계란 메뉴, 고정 4종 주방 스테이션, 주방장·접객 고용 UI와 주급 흐름이 구현돼 있다.
...
- `srcs/day/day_preparation_source.gd`: 생선류·밥·기타요리 스테이션의 순차 재료 수령
- `srcs/day/day_payment.gd`: 좌석 옆 엽전 표시와 접근 회수
- `srcs/day/day_server.gd`: 주문 접수, 완성품 예약·서빙, 결제 회수
- `srcs/day/day_chef.gd`: 메뉴별 고정 스테이션 동선과 조리대 자동 제작
- `srcs/day/day_interaction_controller.gd`: 접근 대상의 우선순위·거리 선택과 진입·이탈 처리
- `srcs/day/cooking_counter.gd`: 모든 메뉴가 공유하는 최종 조리대의 제작 진행, 일시정지와 주문별 접시 완성
...
- 낮 영업 상태는 30초마다 같은 단일 슬롯에 자동 저장되고 낮 단계가 바뀔 때도 즉시 저장된다. 저장된 손님·주문·결제와 중단된 접객 배달은 화면 재생성 시 안전하게 복구한다. 새벽 시장과 준비 중에는 저장 파일을 갱신하지 않는다.
- 손님은 240px/s로 입장·퇴장하며, 좌석 1이 차 있으면 3초 간격으로 최대 3명이 입구에 대기한다. 결제 손님이 퇴장하면 대기열의 첫 손님이 좌석으로 이동하고 새 손님이 맨 뒤를 채운다.
- 낮 주방 스테이션은 메뉴 수와 무관하게 `생선류`, `밥`, `기타요리`, `조리대` 4개로 고정한다. 고등어 초밥은 `생선류 → 밥 → 조리대`, 계란 초밥은 `기타요리 → 밥 → 조리대`를 거치며, 튀김처럼 중간 준비가 필요 없는 메뉴는 `조리대`만 거친다. 음식은 항상 최종 조리대에서 완성된다.
- 고등어 메뉴 Lv.2 강화 비용은 12문이다. 오른쪽 아래 해금 UI에서 구매하면 제작시간이 3.2초에서 3.0초로 줄고, 이후 생성된 주문의 판매가는 6문에서 7문으로 오른다.
- 좌석 2의 비용은 24문이다. 해금 UI에서 구매하면 즉시 설치되고, 대기열의 첫 손님이 새 좌석에 우선 배정돼 이후 두 손님의 주문을 동시에 받을 수 있다.
- Day 2부터 같은 UI에서 좌석 3을 65문, 좌석 4를 140문에 순서대로 구매한다. 설치 즉시 충돌·이동 경로와 손님 배정에 반영된다.
- Day 2부터 계란 메뉴를 80문에 해금하고 40문, 90문을 지불해 Lv.3까지 강화할 수 있다. 제작시간과 판매가는 Lv.1의 4.0초·10문에서 Lv.3의 3.2초·15문까지 변한다.
- 오른쪽 아래 `해금` 버튼에서 좌석과 메뉴를 구매·강화한다. 좌석은 구매 즉시 활성화되고, 메뉴는 해금 직후 준비 재고가 있는 신규 손님의 주문 후보에 들어간다.
- 계란 해금 다음 새벽부터 5인분을 8문에 구매해 `바구니 → 화로 → 식히기 → 보관함` 순서로 준비한다. 해금 후 재고가 있으면 신규 손님의 약 30%가 계란을 주문하며, 낮에는 `기타요리 → 밥 → 조리대` 순서로 제작한다.
- 낮 화면 왼쪽 아래 `직원` 버튼에서 주방장과 접객을 각각 한 명 고용한다. 주방장은 받은 주문을 메뉴 동선대로 자동 조리해 조리대에 완성하고, 접객은 주문 접수·조리대 수령·서빙·돈 계산을 맡는다.
- 주방장 주급은 60문, 접객 주급은 45문이다. 첫 주급은 고용 시 선불로 내고 이후 7일마다 자동 지급한다. 지급일에 돈이 부족하면 해당 직원이 퇴사한다.
- 영업 중 메뉴 강화·좌석·직원 첫 주급을 결제할 때는 다음 날 쌀과 고등어 5인분을 살 수 있도록 최소 10문을 남긴다. 밑천이 부족하면 결제하지 않는다.
...
- 실제 이동 회귀는 고등어 4접시의 주문·생선류·밥·조리·서빙·결제를 거쳐 운영자금 10문을 남기고 첫 메뉴 강화를 60초 안에 구매하는지 검증한다.
- 성장 경로 회귀는 실제 이동으로 14접시를 판매하며 고등어 메뉴 Lv.2와 좌석 2를 차례로 구매하고, 운영자금 10문을 남긴 첫 접객 고용이 게임 시간 5분 안에 가능한지 검증한다.
- 직원 회귀는 주방장과 접객을 함께 고용했을 때 플레이어 작업 없이 주문·4종 스테이션 조리·서빙·결제가 한 판매로 이어지는지 검증한다.
...
| 낮 주방 | 생선류·밥·기타요리·조리대 4종 고정, 메뉴별 경로만 추가 |
| 직원 | 주방장·접객 각 1명, 첫 주급 선불 후 7일마다 지급 |
| 전투 | 없음 |
