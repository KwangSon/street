...
고등어 주문은 `주문 수령 → 고등어 상자 → 밥통 → 고등어 조리대` 순서를 지켜 한 접시씩 처리한다. 앞 단계를 건너뛴 시설 접근은 상태나 재고를 바꾸지 않는다. 제작이 끝나면 주인공이 주문과 연결된 접시를 즉시 든다.
...
- 밥이 부족하면 고등어 상자에서도 고등어가 차감되지 않아 불완전한 준비 상태를 만들지 않는다.
- 고등어는 재료 상자, 밥은 밥통에 접근할 때 각각 한 인분씩 차감된다.
- 작업 범위를 떠나면 진행 중인 접시의 게이지는 멈추며, 다시 접근하면 소비한 재료와 진행도를 유지한 채 이어서 만든다.
- 완성된 접시는 해당 주문과 연결된 상태로 주인공이 즉시 들며, 서빙 전에는 다음 주문을 받을 수 없다.
