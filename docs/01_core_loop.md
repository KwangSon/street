...
영업 중에는 재료 상자와 밥통을 오가며 재료를 따로 들지 않는다. 조리대 작업을 시작할 때 접시 1개분의 준비 재료를 한 번에 소비한다. 새벽의 구매·손질·운반은 영업과 분리된 준비 단계다.
...
- 재료 둘 중 하나라도 부족하면 어느 재료도 차감하지 않고 제작을 시작하지 않는다.
- 작업 범위를 떠나면 진행 중인 접시의 게이지는 멈추며, 다시 접근하면 소비한 재료와 진행도를 유지한 채 이어서 만든다.
- 완성된 접시는 조리대에 쌓이고, 빈손으로 다시 접근하면 한 접시를 먼저 든다.
