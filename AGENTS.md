...
- The current implementation includes the `srcs/main.tscn` bootstrap, GDScript screen switching in `Main`, a Day-screen-only top-left pause menu with confirmed single-slot new-game replacement, 30-second Day autosaves, `LoadingScreen`, the code-built `DayScreen` and `DawnScreen`, JSON save loading, the complete P0 Day 1 loop through settlement, dawn market purchase/refund, ordered batch preparation, the Day 2 transition, purchasable Seats 3–4, the egg unlock and Lv.1–3 production loop, four fixed kitchen stations, chef/service hiring with weekly wages, first-claim employee reservations, and the Stage 01 settlement purchase that resets into the wider Stage 02 location while retaining only menu unlocks and levels.
...
- `DayScreen` builds the Stage-aware gray map, using 1200×1920 for Stage 01 and the wider 1600×2400 layout for Stage 02, plus tap-to-move input, player collision, fixed HUD, bounded drag camera, interaction selection, the four fixed kitchen stations, the bottom-left employee UI, and the bottom-right unlock/upgrade UI entirely from GDScript.
...
- Player and employee work uses first-claim, non-preemptive ownership. Employees reserve orders, counter plates, and payments before moving; the player claims work on interaction. A later arrival cannot steal a claim, and path failure, employee departure, or load recovery releases transient employee claims.
...
- Stage 01 settlement offers the Stage 02 location for 500 mon. Purchasing rebuilds the Day screen with the wider layout and starts Stage 02 Day 1 with zero currency, one seat, no employees, reset runtime/totals, and starter rice/mackerel; only mackerel and egg menu unlock/level state persists.
