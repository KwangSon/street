...
class_name DayStaffHirePad

const PURCHASE_DURATION: float = 1.0
const INTERACTION_MARGIN: float = 28.0
const PAD_SIZE: Vector2 = Vector2(180.0, 96.0)
const PAD_COLOR: Color = Color("8a769d")
...
func _build_visual() -> void:
	...
	_title_label.position = Vector2(-88.0, -43.0)
	_title_label.size = Vector2(176.0, 36.0)
	...
	_status_label.position = Vector2(-88.0, -5.0)
	_status_label.size = Vector2(176.0, 34.0)
	...
	_progress_bar.position = Vector2(-80.0, 32.0)
...
func _refresh_visual() -> void:
	if _body == null:
		return
	var completed: bool = GameManager.is_server_hired()
	...
	var hire_cost: int = GameManager.get_server_hire_cost()
	var currency: int = int(GameManager.state.get("currency", 0))
	if completed:
		_status_label.text = "완성품 자동 서빙"
		_status_label.add_theme_font_size_override("font_size", 15)
		...
		_status_label.text = "%d문 · 1초 머물기" % hire_cost
		_status_label.add_theme_font_size_override("font_size", 15)
		_status_label.add_theme_color_override(
			"font_color",
			TEXT_COLOR
